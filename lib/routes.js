module.exports = function () {
    function create(app, config, io) {
        ...
        function getFormData(key, req) {
            var tuples = [];
            if (debug) console.log('content-type:', req.xContentType);
            if (req.xContentType === 'application/x-www-form-urlencoded') {
                ...
                if (tuples.length === 1 && !tuples[0].value) {
                    if (debug) console.log('malformed-tuple:', tuples);
            ...
            if (debug) console.log('tuples:', tuples);
            return tuples;
        }
